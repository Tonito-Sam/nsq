    ...
    queryFn: async () => {
      try {
        ...
        const { data: storesData, error: storesError } = await supabase
          .from('user_stores')
          .select('id, store_name, verification_status, is_active, user_id')
          .in('id', storeIds);
        ...
        // Fetch store owners/users in batch if available
        const ownerIds = Array.from(new Set((storesData || []).map((s: any) => s.user_id).filter(Boolean)));
        ...
        if (ownerIds.length > 0) {
          ...
          if (!ownersError && ownersData) {
            ownersById = (ownersData || []).reduce((acc: any, u: any) => ({ ...acc, [u.id]: u }), {});
          } else {
            console.warn('Could not fetch store owners:', ownersError);
          }
        }
        ...
        return typedProducts.map(product => {
          ...
          const owner = storeData && storeData.user_id ? ownersById[storeData.user_id] : null;
          return {
            ...product,
            store: storeData
              ? {
                  ...
                  id: storeData.id,
                  // attach owner info so UI can show seller name
                  owner: owner ? {
                    id: owner.id,
                ...
                <div className="flex flex-col justify-center space-y-6 z-20 order-last lg:order-first">
                  ...
                  <div className={`transition-all duration-1000 delay-700 ${
                    ...
                    <div className="text-lg sm:text-xl text-gray-600 dark:text-gray-300">
                      <span className="font-semibold text-purple-600 dark:text-purple-400">Sold by:</span> {p.store?.owner?.username || p.store?.store_name || 'Seller'}
                    </div>
                  </div>
                              ...
                              <td className="py-3 px-4">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-xs text-gray-500 dark:text-gray-400">{product.store?.owner?.username || product.store?.store_name || 'Unknown Store'}</div>
                                  </div>
                                </div>
                              </td>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="mt-2">
                                ...
                                <div className="text-xs text-gray-500 mt-1">{p.store?.owner?.username || p.store?.store_name || 'Seller'}</div>
                              ...
                              {pageItems.map(p => {
                                ...
                                return (
                                  <div key={p.id} className="bg-white dark:bg-[#161616] rounded-lg p-4 shadow-sm hover:shadow-md transition cursor-pointer flex" onClick={() => navigate(`/product/${p.id}`)}>
                                    ...
                                    <div className="flex-1">
                                      <div className="text-sm font-semibold truncate" title={p.title}>{p.title}</div>
                                      <div className="text-xs text-gray-500 mb-2">{p.store?.owner?.username || p.store?.store_name || 'Seller'}</div>
                                      <div className="flex items-center justify-between">
