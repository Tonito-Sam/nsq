...
const Studio = () => {
  ...
  // Batch video loading and eager fetch of channel/author data
  const [videoBatch, setVideoBatch] = useState(0); // batch index
  const BATCH_SIZE = 10;
  useEffect(() => {
    setVideoBatch(0); // reset batch on category change
  }, [selectedCategory]);
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      let query = supabase
        ...
        .order('created_at', { ascending: false })
        .range(videoBatch * BATCH_SIZE, (videoBatch + 1) * BATCH_SIZE - 1);
      ...
      setVideos(prev => videoBatch === 0 ? videosWithChannelName : [...prev, ...videosWithChannelName]);
  ...
  }, [selectedCategory, videoBatch, highlightId]);

  // Infinite scroll for batches (mobile only)
  useEffect(() => {
    if (!videos.length || isDesktop) return;
    const handleScroll = () => {
      if ((window.innerHeight + window.scrollY) >= (document.body.offsetHeight - 300)) {
        setVideoBatch(batch => batch + 1);
      }
    };
  ...
  const handleLike = async (videoId: string) => {
    ...
    // Update videos state optimistically
    ...
    try {
      ...
      // Refetch videos to ensure consistency
