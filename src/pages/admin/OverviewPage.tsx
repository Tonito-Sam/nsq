...
const OverviewPage = () => {
  ...
  const fetchContentStats = async () => {
    ...
    // Studio channels, subscribers and reels
    try {
      const { count: totalStudios } = await supabase
        .from('studio_channels')
        .select('id', { count: 'exact', head: true });

      const { count: totalStudioSubscribers } = await supabase
        .from('studio_channel_subscribers')
        .select('id', { count: 'exact', head: true });

      // Reels are stored in the `studio_videos` table
      ...
      setStats(prev => ({
        ...
        totalStudioSubscribers: totalStudioSubscribers || 0,
        totalReels: totalReels || 0
      }));
    } catch (err) {
      console.warn('Error fetching studio/reel stats:', err);
    }
  };
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="text-center p-3 border rounded-lg">
                <div className="text-xl font-bold">{stats.totalStudios.toLocaleString()}</div>
                <p className="text-xs text-muted-foreground">Studio Channels</p>
              </div>
              <div className="text-center p-3 border rounded-lg">
                <div className="text-xl font-bold">{stats.totalStudioSubscribers.toLocaleString()}</div>
                <p className="text-xs text-muted-foreground">Total Subscribers</p>
              </div>
              <div className="text-center p-3 border rounded-lg">
                <div className="text-xl font-bold">{stats.totalReels.toLocaleString()}</div>
